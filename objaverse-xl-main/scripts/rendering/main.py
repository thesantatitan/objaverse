...
def handle_found_object(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create the renders directory in the target location
        fs, path = fsspec.core.url_to_fs(render_dir)
        fs.makedirs(os.path.join(path, "renders", save_uid), exist_ok=True)
        
        # Copy all files to the render directory
        for file_path in png_files + metadata_files + npy_files:
            target_path = os.path.join(path, "renders", save_uid, os.path.basename(file_path))
            fs.put(file_path, target_path)
...
def render_objects(
    render_dir: str = "~/.objaverse_new",
    download_dir: Optional[str] = "~/.objaverse_new",
    num_renders: int = 12,
    processes: Optional[int] = 16,
